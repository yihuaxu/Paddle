...
#include "paddle/fluid/framework/ir/conv3d_relu_mkldnn_fuse_pass.h"
...
TEST(Conv3DReLUFusePass, basic) {
  auto prog = BuildProgramDesc();

  std::unique_ptr<ir::Graph> graph(new ir::Graph(prog));

  auto pass = PassRegistry::Instance().Get("conv3d_relu_mkldnn_fuse_pass");
  ...
  // Remove 3 Nodes: CONV, RELU, conv3d_out
  // Add 1 Node: ConvReLU
  EXPECT_EQ(original_nodes_num - 2, current_nodes_num);

  // Assert conv3d_relu op in newly generated graph
  int conv3d_relu_count = 0;
...
USE_PASS(conv3d_relu_mkldnn_fuse_pass);
